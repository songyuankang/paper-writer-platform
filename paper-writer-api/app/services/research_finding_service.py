...
class ResearchFindingService:
 def __init__(self,settings:Settings): self.settings=settings; self.analyses=AnalysisService(settings); self.explanations=ResearchExplanationService(settings); self.root=settings.db_path.parent/"findings"; self.root.mkdir(parents=True,exist_ok=True)
 ...
 def _references(self,task_id:str,result_id:str):
  ...
  for section in draft.get("sections",[]):
   for block in section.get("paragraphs",[]):
    ...
    entry={"id":block.get("id"),"label":block.get("title") or block.get("id"),"title":block.get("title") or ""}
    (figures if block.get("type")=="chart" else tables if block.get("type")=="table" else []).append(entry)
