...
export default function ResearchAnalysis() {
  ...
  async function runNewAnalysis() {
    ...
    try {
      const variables = type === "descriptive" ? { columns: columns.length ? columns : schema.map((item) => item.name) } : type === "independent_t" || type === "anova" ? { group_column: groupColumn, value_column: valueColumn } : { x, y };
