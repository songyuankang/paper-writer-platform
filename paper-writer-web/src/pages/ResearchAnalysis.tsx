...
export default function ResearchAnalysis() {
  ...
  const [explanation, setExplanation] = useState<ResearchExplanation | null>(null);
  const [finding, setFinding] = useState<ResearchFinding | null>(null);
  const [findingLength, setFindingLength] = useState("standard");
  ...
  async function runNewAnalysis() {
    ...
    try {
      ...
      setAnalysis(response.analysis); setResult(response.result); setExplanation(null);
  ...
  async function chooseAnalysis(item: Analysis) {
    setAnalysis(item); setResult(null); setExplanation(null); setError("");
    try { if (item.last_result_id) setResult(await getAnalysisResult(item.id)); } catch (err) { setError(message(err, "无法读取分析结果")); }
  }
  async function rerun() {
    if (!analysis) return;
    setBusy(true); setError("");
          try { const response = await runAnalysis(analysis.id); setAnalysis(response.analysis); setResult(response.result); setExplanation(null); setNotice("已重新运行，并保留旧 AnalysisResult。" ); await refresh(); } catch (err) { setError(message(err, "重新运行失败")); } finally { setBusy(false); }

  }
  async function explain() {
    if (!analysis || !result || result.status !== "ready") return;
    setBusy(true); setError("");
    try { setExplanation(await explainAnalysisResult({ analysis_id: analysis.id, analysis_result_id: result.id })); setNotice("AI 解释已生成；统计事实仅来自当前 AnalysisResult。" ); } catch (err) { setError(message(err, "生成 AI 解释失败")); } finally { setBusy(false); }
  }
  async function generateFinding() { if (!analysis || !result || !explanation || !taskId) return; setBusy(true); setError(""); try { setFinding(await createResearchFinding({task_id:taskId,analysis_id:analysis.id,analysis_result_id:result.id,explanation_id:explanation.id,style:{paper_style:"undergraduate",tone:"formal",length:findingLength}})); setNotice("已生成可预览结果段落，尚未写入论文。"); } catch(err) { setError(message(err,"生成结果段落失败")); } finally { setBusy(false); } }
  async function confirmFinding() { if (!finding || !sectionId) return; setBusy(true); try { await insertResearchFinding(finding.id,sectionId); setFinding({...finding,status:"inserted"}); setNotice("ResearchFinding 已作为可追溯正文块插入论文。"); } catch(err) { setError(message(err,"插入结果段落失败")); } finally { setBusy(false); } }
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-900"><header className="border-b border-slate-200 bg-white px-5 py-3 shadow-sm"><div className="mx-auto flex max-w-[1800px] flex-wrap items-center gap-3"><Link to={`/research/data${taskId ? `?task_id=${encodeURIComponent(taskId)}` : ""}`} className="text-sm text-slate-500 hover:text-slate-900">← 研究数据中心</Link><div className="h-5 border-l border-slate-200" /><h1 className="mr-auto text-lg font-semibold">研究分析</h1><input value={taskId} onChange={(event) => setTaskId(event.target.value.trim())} placeholder="32 位论文任务 ID" className="w-64 rounded border border-slate-300 p-2 text-sm" /></div></header>
    <main className="mx-auto grid max-w-[1800px] grid-cols-1 gap-4 p-4 xl:grid-cols-[300px_minmax(500px,1fr)_420px]"><aside className="space-y-4"><section className="rounded-xl border bg-white p-4 shadow-sm"><h2 className="mb-3 font-semibold">数据集</h2><select value={datasetId} onChange={(event) => setDatasetId(event.target.value)} className="mb-2 w-full rounded border border-slate-300 p-2 text-sm"><option value="">选择数据集</option>{datasets.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.row_count} 行</option>)}</select><select value={version || ""} onChange={(event) => setVersion(Number(event.target.value) || undefined)} disabled={!datasetId} className="w-full rounded border border-slate-300 p-2 text-sm disabled:bg-slate-100"><option value="">选择版本</option>{versions.map((item) => <option key={item.version} value={item.version}>Dataset v{item.version} · {item.row_count} 行</option>)}</select><div className="mt-4 space-y-2">{schema.map((field) => <div key={field.name} className="rounded border border-slate-100 bg-slate-50 p-2"><div className="flex justify-between text-sm font-medium"><span>{field.name}</span><span className="text-xs text-slate-500">{field.type}</span></div><p className="mt-1 text-xs text-slate-500">缺失 {field.missing_count} · 唯一 {field.unique_count}</p></div>)}</div></section><section className="rounded-xl border bg-white p-4 shadow-sm"><h2 className="mb-2 font-semibold">历史分析</h2><div className="space-y-2">{analyses.map((item) => <button key={item.id} onClick={() => void chooseAnalysis(item)} className={`w-full rounded border p-2 text-left text-sm ${analysis?.id === item.id ? "border-blue-400 bg-blue-50" : "border-slate-200"}`}><p className="truncate font-medium">{item.name}</p><p className="mt-1 text-xs text-slate-500">{item.type} · v{item.dataset_version} · {item.status}</p></button>) || <p className="text-sm text-slate-500">尚无分析。</p>}</div></section></aside>
      ...
      <aside className="space-y-4"><section className="rounded-xl border bg-white p-4 shadow-sm"><h2 className="mb-3 font-semibold">结果</h2>{result ? <div className="space-y-4">{result.status === "failed" ? <p className="rounded bg-red-50 p-3 text-sm text-red-700">分析未完成：{result.warnings.join("；")}</p> : <>{result.result.method === "descriptive" ? <><h3 className="text-sm font-medium">数值变量</h3><div className="overflow-auto rounded border"><table className="min-w-full text-xs"><thead className="bg-slate-50"><tr><th className="p-2 text-left">变量</th><th className="p-2">N</th><th className="p-2">Missing</th><th className="p-2">Mean</th><th className="p-2">Median</th><th className="p-2">Std</th></tr></thead><tbody>{(result.result.numeric || []).map((item) => <tr key={item.variable} className="border-t"><td className="p-2">{item.variable}</td><td className="p-2 text-center">{item.count}</td><td className="p-2 text-center">{item.missing}</td><td className="p-2 text-center">{number(item.mean)}</td><td className="p-2 text-center">{number(item.median)}</td><td className="p-2 text-center">{number(item.std)}</td></tr>)}</tbody></table></div><h3 className="text-sm font-medium">分类变量</h3>{(result.result.categorical || []).map((item) => <div key={item.variable} className="rounded border p-2 text-sm"><p className="font-medium">{item.variable} · N={item.count} · Missing={item.missing}</p>{item.frequency.map((row) => <p key={row.category} className="mt-1 text-xs text-slate-600">{row.category}: {row.frequency} ({row.percentage.toFixed(2)}%)</p>)}</div>)}</> : <GroupResult result={result} />}{result.warnings.length > 0 && <div className="rounded bg-amber-50 p-3 text-sm text-amber-800">Warning：{result.warnings.join("；")}</div>}</>}</div> : <p className="text-sm text-slate-500">运行或选择历史分析后显示结构化结果。</p>}</section><section className="rounded-xl border bg-white p-4 shadow-sm"><div className="flex items-center gap-2"><h2 className="mr-auto font-semibold">AI 结果解释</h2><button onClick={() => void explain()} disabled={!analysis || result?.status !== "ready" || busy} className="rounded bg-violet-600 px-3 py-2 text-sm text-white disabled:opacity-50">解释结果</button></div><p className="mt-2 text-xs text-slate-500">本区域为 AI 解释，不是新的统计计算；统计事实逐项来自当前 AnalysisResult。</p>{explanation ? <div className="mt-3 space-y-3 text-sm"><p><b>统计结论：</b>{explanation.analysis_summary}</p><div><b>统计事实（系统计算）：</b>{explanation.statistical_facts.map((item, index) => <p key={index} className="mt-1 rounded bg-slate-50 p-2 text-xs">{item.text}</p>)}</div><div><b>AI 解释：</b>{explanation.interpretation.map((item, index) => <p key={index} className="mt-1">{item}</p>)}</div>{explanation.limitations.length > 0 && <div className="rounded bg-amber-50 p-2 text-amber-900"><b>限制：</b>{explanation.limitations.join("；")}</div>}{explanation.cautions.length > 0 && <div className="rounded bg-blue-50 p-2 text-blue-900"><b>注意：</b>{explanation.cautions.join("；")}</div>}<p className="text-xs text-slate-500">来源：{explanation.provider} · Dataset v{explanation.dataset_version} · {explanation.data_fingerprint.slice(0, 12)} · {new Date(explanation.created_at).toLocaleString()}</p></div> : <p className="mt-3 text-sm text-slate-500">选择一个已完成的分析结果后，可请求结构化解释。</p>}</section><section className="rounded-xl border bg-white p-4 shadow-sm"><h2 className="font-semibold">生成论文结果</h2><p className="mt-1 text-xs text-slate-500">依据已验证 Explanation 与真实 AnalysisResult 生成；不会自动插入论文。</p><select value={findingLength} onChange={(e)=>setFindingLength(e.target.value)} className="mt-3 w-full rounded border p-2 text-sm"><option value="concise">简洁</option><option value="standard">标准</option><option value="detailed">详细</option></select><button onClick={()=>void generateFinding()} disabled={!explanation||busy} className="mt-3 rounded bg-slate-900 px-3 py-2 text-sm text-white disabled:opacity-50">生成结果段落</button>{finding&&<div className="mt-3 space-y-2 text-sm"><b>{finding.title}</b>{finding.paragraphs.map((p,i)=><p key={i} className="rounded bg-slate-50 p-2">{p}</p>)}<p className="text-xs text-slate-500">表：{finding.table_references.map(x=>x.label).join("、")||"无"}；图：{finding.figure_references.map(x=>x.label).join("、")||"无"}</p><button onClick={()=>void confirmFinding()} disabled={finding.status==="inserted"||busy} className="rounded bg-blue-600 px-3 py-2 text-sm text-white disabled:opacity-50">{finding.status==="inserted"?"已插入论文":"插入论文"}</button></div>}</section>{(notice || error) && <div className={`rounded-xl border p-3 text-sm ${error ? "border-red-200 bg-red-50 text-red-700" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>{error || notice}</div>}</aside>
    </main></div>;
}
