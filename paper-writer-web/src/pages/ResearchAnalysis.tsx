...
export default function ResearchAnalysis() {
  ...
  async function runNewAnalysis() {
    ...
    try {
      const variables = type === "descriptive" ? { columns: columns.length ? columns : schema.map((item) => item.name) } : { x, y };
      if (type !== "descriptive" && (!x || !y)) throw new Error("请选择 X 与 Y 两个数值变量。");
  ...
  async function insert(artifact: "table" | "chart") {
