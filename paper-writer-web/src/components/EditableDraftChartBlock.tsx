...
const COLORS = ["#4f6fbe", "#70ad47", "#e9ad38", "#d96a68"];

function text(value: unknown, fallback = "") {
  return typeof value === "string" ? value : fallback;
}

function ChartCanvas({ block }: { block: DraftParagraph }) {
  ...
  return <svg viewBox={"0 0 " + width + " " + height} className="h-auto w-full min-w-[560px]" role="img" aria-label={text(block.title, "图表预览")}>
    ...
    {categories.map((category, index) => <text key={category + index} x={left + categoryWidth * index + categoryWidth / 2} y={height - 22} textAnchor="middle" fontSize="11" fill="#5f6368">{category}</text>)}
    {series.map((item, seriesIndex) => {
      const isLine = kind === "line" || (kind === "mixed" && seriesIndex > 0);
      ...
      if (isLine) return <g key={item.name}><polyline points={points.map((point) => point.join(",")).join(" ")} fill="none" stroke={color} strokeWidth="2.5" />{points.map((point, index) => <g key={index}><circle cx={point[0]} cy={point[1]} r="3.5" fill="white" stroke={color} strokeWidth="2" /><text x={point[0]} y={point[1] - 9} textAnchor="middle" fontSize="10" fill={color}>{item.values[index]}</text></g>)}</g>;
      const barCount = Math.max(1, series.filter((_, i) => kind !== "mixed" || i === 0).length);
      const barWidth = Math.min(46, categoryWidth * 0.56 / barCount);
      return <g key={item.name}>{item.values.map((value, index) => { const barHeight = (value / maximum) * plotHeight; const x = left + categoryWidth * index + categoryWidth / 2 - barWidth / 2 + (seriesIndex * barWidth); const y = top + plotHeight - barHeight; return <g key={index}><rect x={x} y={y} width={barWidth} height={barHeight} rx="2" fill={color} /><text x={x + barWidth / 2} y={y - 7} textAnchor="middle" fontSize="10" fill={color}>{value}</text></g>; })}</g>;
    })}
  </svg>;
}

export default function EditableDraftChartBlock({ block, index, onUpdate, onRegenerate, onDelete, onMove, canMoveUp, canMoveDown }: Props) {
  ...
  const provenance = block.provenance === "user_provided" ? "来源：用户数据" : block.provenance === "model_generated" ? "来源：模型生成" : "数据示意";
  const label = useMemo(() => ({ bar: "柱状图", line: "折线图", mixed: "柱线混合图", pie: "饼图" }[block.chart?.kind ?? "bar"]), [block.chart?.kind]);
  ...
  return <article className="my-5 overflow-hidden rounded-lg border border-neutral-200 bg-white shadow-sm" data-chart-block={block.id}>
    <div className="flex flex-wrap items-center gap-2 border-b border-neutral-200 bg-neutral-50 px-3 py-2 text-xs text-neutral-600">
      ...
      <button type="button" onClick={regenerate} disabled={busy} className="rounded px-2 py-1 hover:bg-neutral-200 disabled:opacity-50">{busy ? "生成中…" : "↻ 重新生成"}</button>
    ...
    <div className="mx-auto" style={{ width: String(scale * 100) + "%", minWidth: "min(100%, 560px)" }}><div className="px-5 pt-5 text-center"><h4 className="text-base font-semibold text-neutral-900">{block.title}</h4></div><ChartCanvas block={block} /><div className="px-5 pb-4 text-center"><p className="text-sm font-medium text-neutral-800">图 {index + 1} {block.title}</p><p className="mt-1 text-xs text-neutral-500">{block.caption}</p>{block.provenance !== "user_provided" && <p className="mt-1 text-xs text-amber-700">数据为模型/示意生成，未自动检索或核验外部来源。</p>}</div></div>
  </article>;
}
